...
class TestGetStepArray(unittest.TestCase):
    def test_get_step_array(self):
        self.assertEqual(get_step_array([0], 0, 0, 1), [1])

        self.assertEqual(get_step_array([0, 0, 0, 0], 0, 0, 1), [4])

        self.assertEqual(get_step_array([1, 2, 3, 4, 5], 1, 5, 1), [1, 1, 1, 1, 1])

        self.assertEqual(get_step_array([1, 1, 2, 2], 1, 2, 1), [2, 2])

        self.assertEqual(get_step_array([1, 1, 0, 3, 3], 0, 3, 1), [1, 2, 0, 2])

        self.assertEqual(
            get_step_array([-4, -4, 2, 5], -4, 5, 1), [2, 0, 0, 0, 0, 0, 1, 0, 0, 1]
        )

        self.assertEqual(get_step_array([1.1, 1.4], 1.1, 1.4, 0.1), [1, 0, 0, 1])
